...
import type { Session, TrackRecord } from '../../lib/tracker/types';
import {
  VideoSource,
  closeVideo,
  openVideo,
  ...
  const importCsv = async (files: FileList) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        added.push(sessionFromCsv(text, file.name));
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e));
      }
    }
  ...
  return (
    <div className="kt-wrap">
      <div className="kt-header">
        ...
        <p>
          Measure knee stability from a video. Mark an anatomical landmark, and the app tracks it
          across every frame with Lucas–Kanade optical flow, graphing how far it deviates over time.
        </p>
      ...
      {error && <div className="kt-error">{error}</div>}
      ...
      <div className="kt-grid">
        ...
        <div>
          {phase === 'empty' ? (
            ...
            >
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="1.6" style={{ marginBottom: 8 }}>
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                <polyline points="17 8 12 3 7 8" />
                <line x1="12" y1="3" x2="12" y2="15" />
              </svg>
              <div style={{ fontWeight: 600, color: '#374151' }}>
                Drop a knee video here, or click to choose a file
              </div>
              <div className="kt-hint" style={{ marginTop: 6 }}>
                MP4 / MOV / WebM · processed locally, nothing is uploaded
              </div>
            </div>
          ) : (
            <div className="kt-card">
              ...
              {phase === 'setup' && (
                <>
                  ...
                  />
                  <div className="kt-row" style={{ justifyContent: 'space-between' }}>
                  ...
                  {markMode === 'mark' ? (
                    ...
                    <div>
                      ...
                      <div className="kt-row">
                        ...
                        {pxPerMm && (
                          <span className="kt-hint" style={{ color: '#047857' }}>
              ...
              {phase === 'tracking' && (
                <>
                  ...
                  {(paused || lost) && (
                    <div className="kt-hint" style={{ marginTop: 6, color: lost ? '#b91c1c' : '#6b7280' }}>
                      {lost
          ...
          {/* results below the stage */}
          {!compareMode && selected && (
            <div className="kt-card" style={{ marginTop: '1.25rem' }}>
              <div className="kt-row" style={{ justifyContent: 'space-between', marginTop: 0 }}>
                <h3 style={{ margin: 0 }}>{selected.name}</h3>
                <button className="kt-btn kt-btn-ghost kt-btn-sm" onClick={() => exportCsv(selected)}>
                  Download CSV
                </button>
              </div>
              <DeviationChart session={selected} />
            </div>
          )}
          ...
          {compareMode && compareList.length < 2 && (
            <div className="kt-card" style={{ marginTop: '1.25rem' }}>
              <p className="kt-hint">Select at least two sessions on the right to compare them.</p>
            </div>
          )}
        </div>

        {/* -------------------------------------------------- right: settings/sessions */}
        <div>
          <div className="kt-card">
            <h3>Capture settings</h3>
            <div className="kt-field">
              <label className="kt-label">Camera view</label>
              <select
                className="kt-select"
                value={view}
            ...
            <div className="kt-field">
              <label className="kt-label">Tracking profile</label>
              <select
                className="kt-select"
                value={presetName}
            ...
            <div className="kt-field">
              <label className="kt-label">Reference position</label>
              <select
                className="kt-select"
                value={referenceMode}
            ...
            {phase !== 'empty' && (
              <div className="kt-field">
                <label className="kt-label">Frame rate (fps)</label>
                <input
                  className="kt-input"
                  type="number"
                  value={fps}
          ...
          <div className="kt-card" style={{ marginTop: '1.25rem' }}>
            <div className="kt-row" style={{ justifyContent: 'space-between', margin: 0 }}>
            ...
            {sessions.length === 0 ? (
              <p className="kt-hint" style={{ marginTop: 8 }}>
                Tracked clips appear here. Track two or more, then compare them on one graph.
              </p>
            ) : (
              <>
                ...
                <ul className="kt-sessions" style={{ marginTop: 8 }}>
                  {sessions.map((s, i) => (
                    <li
                      key={s.id}
